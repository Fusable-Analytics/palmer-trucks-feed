...
for item in data:
    ...
    if condition and condition.upper() in ["NEW", "USED", "CPO"]:
    ...
    if not (make and model and year and price and image_urls and description):
        continue

    # VIN fallback URL setup
    vin_url = f"https://www.palmertrucks.com/truck-details/category-{category.replace(' ', '-')}/make-{make}/model-{model}/vin-"
    if not vin:
        print(f"⚠️ Skipping missing VIN for stock: {stock_number}")
        continue
    else:
        vin_url += vin + "/"
    ...
    # --- Begin building XML entry ---
    listing = ET.SubElement(root, "listing")

    # All images (main + up to 20 additional) as individual <image> elements
    for idx, img_url in enumerate(image_urls[:21]):
        image = ET.SubElement(listing, "image")
        ET.SubElement(image, "url").text = img_url
        if idx == 0:
            ET.SubElement(image, "tag").text = category  # optional, only on primary
